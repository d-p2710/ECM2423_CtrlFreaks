...
class Crossword(object):
    def __init__(self, cols, rows, empty='-', maxloops=2000, available_words=[]):
        ...
        self.available_words = available_words # list of ∀. words that can potentially be placed in the crossword
...
class Word(object):
    def __init__(self, word=None, clue=None, level=None):
        self.word = re.sub(r'\s', '', word.lower())
        self.clue = clue
        self.level =level
        self.length = len(self.word)

        # Set when placed on board
        self.row = None
        self.col = None
        self.vertical = None
        self.number = None
